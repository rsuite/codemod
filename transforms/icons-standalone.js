...
module.exports = (file, api, options) => {
  ...
  root
    ...
    .forEach((rsuiteImport) => {
          ...
          root
            ...
            .forEach((IconTag) => {
              ...
              if (j.StringLiteral.check(iconAttributeValue.value)) {
                ...
                j.StringLiteral.check(
                  iconAttributeValue.value.expression.alternate
                )
              ) {
                ...
                const isInsideExpressionContainer =
                  j.JSXExpressionContainer.check(
                    IconElementPath.parentPath.value
                  );
      ...
      if (j(rsuiteImport).find(j.ImportSpecifier).size() < 1) {
        j(rsuiteImport).remove();
      }
    });
